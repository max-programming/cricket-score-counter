import "./Controls.css";

const Controls = () => {
  const wicketImgURL =
    "data:image/svg+xml;base64,";
  return (
    <div className="controls">
      <button className="add-btn dot">0</button>
      <div className="add-runs">
        <button className="add-btn one">1</button>
        <button className="add-btn two">2</button>
        <button className="add-btn three">3</button>
        <button className="add-btn four">4</button>
        <button className="add-btn five">5</button>
        <button className="add-btn six">6</button>
      </div>
      <div className="add-wicket">
        <button className="wicket-btn">
          <img src={wicketImgURL} width="20" height="20" alt="wickets" />
          &nbsp; WICKET!
        </button>
      </div>
    </div>
  );
};

export default Controls;
